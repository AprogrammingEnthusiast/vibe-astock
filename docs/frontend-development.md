...
- 所有后端地址经过 `frontend/src/lib/base.ts` 的 `apiUrl()`。VR 请求扩展 `lib/api.ts`，复盘请求复用 `lib/agent.ts` 的 agentFetch/agentPost，对话复用 `lib/llm.ts` 的 chatStream；保留鉴权、账号绑定与 CSRF 行为，避免页面各写一套请求客户端。
...
- 主题切换复用 `frontend/src/hooks/useDarkMode.ts`：CSS 根变量是暗色，`.light` 覆盖为亮色，但 hook 在无保存偏好时默认浅色。两种主题都要检查正文、表格、表单和图表对比度，不能仅按 CSS 注释判断实际默认主题。
- 页面优先复用 PageHeader、GlassCard、Caliber、Disclaimer 等现有组件；类名组合使用 `cn()`，图标使用 lucide-react。主内容沿用 Layout 的 max-w-6xl 与页边距，面板按需响应式分栏；窄屏长表和复杂图表在自身容器横向滚动，避免挤坏整页。
