...
function pendingDraft(): {id:string;connection:AgentConnection}|null {
 try {
  const value=JSON.parse(sessionStorage.getItem(accountKey(pendingKey))||'null');const c=value?.connection;
...
export function AgentAccess({initialProvider,onSaved,onBusyChange}:{initialProvider?:string;onSaved?:()=>void;onBusyChange?:(busy:boolean)=>void}) {
 ...
 const busy=sending||job.status==='running'||saving.current;
 ...
 useEffect(()=>{
  ...
  const poll=async()=>{
   ...
   try{
    ...
    if(!abort.signal.aborted&&epoch===pollEpoch.current){setPollError('');void reconcile(next);}
 ...
 async function reconcile(next:Job){
  if(saving.current)return;
  if(pending.current&&next.id!==pending.current.id)return;
  if(pending.current||next.status!=='running')setUnconfirmed(false);
  setJob(next);
    if((next.status==='failed'||next.status==='cancelled')&&pending.current?.id===next.id){pending.current=null;sessionStorage.removeItem(accountKey(pendingKey));}
    ...
    if(next.status==='complete' && pending.current && pending.current.id===next.id){
     ...
     saving.current=true;
     try { await saveAgentConnection({...connection,verifiedAt:Date.now()}); }
     catch { setError("连接测试已通过，但个人配置保存失败，请刷新后重试保存"); return; }
     finally { saving.current=false; }
     pending.current=null;sessionStorage.removeItem(accountKey(pendingKey));
 ...
 async function start(kind:'login'|'probe',retry=false){
  ...
  try{
   ...
   if(kind==='probe'){
    ...
    pending.current={id:requestId,connection};sessionStorage.setItem(accountKey(pendingKey),JSON.stringify(pending.current));
   ...
   if(dispatched&&kind==='probe'&&pending.current&&!(e instanceof AgentRequestError&&e.status<500)){
    ...
    pollEpoch.current+=1;pending.current=null;sessionStorage.removeItem(accountKey(pendingKey));setJob({status:'failed'});
