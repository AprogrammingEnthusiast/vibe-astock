...
export function QuickAiConnect() {
  ...
  return <Dialog titleId="quick-ai-title" close={()=>{if(!accessBusy)state.closeConnect();}} className="max-w-4xl max-h-[90dvh] overflow-auto">
    <div className="p-6 sm:p-7">
      <div className="sticky top-0 z-10 -mx-6 -mt-6 flex items-start justify-between gap-3 border-b border-border bg-card px-6 py-3 sm:-mx-7 sm:-mt-7 sm:px-7">
      ...
      <button type="button" disabled={accessBusy} onClick={() => setSelected('api')} className="mt-3 w-full rounded-xl border border-border p-3 text-left text-sm">连接 API · DeepSeek、通义、Kimi 等</button>
