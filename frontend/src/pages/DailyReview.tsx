...
export function DailyReview() {
  ...
  const [autoRefresh, setAutoRefresh] = useState<boolean>(
    () => { try { return localStorage.getItem(accountKey(AUTO_KEY)) === "1"; } catch { return false; } });
  // 关注股票（自选，存本地）
  ...
  const toggleAuto = () => {
    ...
    try { localStorage.setItem(accountKey(AUTO_KEY), next ? "1" : "0"); setAutoRefresh(next); setWatchError(null); }
  ...
  const addWatch = async () => {
    ...
    setWatchInput("");
    if (!added) return;
    await saveWatch(next); setWatchCodes(next); refreshWatch(next); setWatchError(null);
