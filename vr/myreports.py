...
- 存到 `VR_REPORTS_DIR`（默认 ~/.vibe-astock-agent/market-data/myreports/，也可用 VR_DATA_DIR 换根目录）——用户私有资料，绝不进仓、不上传。
...
_DATA_DIR = Path(os.environ.get("VR_DATA_DIR") or Path.home() / ".vibe-astock-agent" / "market-data")
